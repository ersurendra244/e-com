...
class CategoryController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $query = Category::whereNull('parent_id')->where('is_delete', '0');
    ...
    public function subcategory_list(Request $request)
    {
        ...
        // Base query
        $query = Category::with('parents')->whereNotNull('parent_id')->where('is_delete', '0');
        // Count total
        $totalData = $query->count();
    ...
    public function subcategory_edit(Request $request, $slug)
    {
        $data['title'] = 'Edit Subcategory';
        $data['subtitle'] = 'Masters';
        $data['edit_data'] = Category::whereNotNull('parent_id')->where('slug', $slug)->first();
        $data['categories'] = Category::whereNull('parent_id')->get();
    ...
    public function subcategory_update(Request $request, $slug)
    {
        $modal = Category::whereNotNull('parent_id')->where('slug', $slug)->firstOrFail();
        $validator = Validator::make($request->all(), [
            'name' => 'required|unique:categories,name,' . $modal->id,
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
        ]);
        if ($validator->fails()) {
            return redirect()->back()->withErrors($validator)->withInput();
    ...
    public function subcategory_delete($slug)
    {
        $modal = Category::whereNotNull('parent_id')->where('slug', $slug)->first();
        if ($modal) {
            $modal->status = '0';
            $modal->is_delete = '1';
            $modal->save();
