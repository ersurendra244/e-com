...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <a href="{{ roleRoute('users') }}" class="btn btn-sm btn-dark float-right">Go Back</a>
                    <h4 class="card-title">{{ $title }}</h4>
                    <form action="{{ roleRoute('users.update', ['id' => $user->id]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                    ...
                                    <select name="role" class="form-control" required>
                                        ...
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}"
                                                {{ isset($user->roles) && $user->roles->contains('id', $role->id) ? 'selected' : '' }}>
                                                {{ $role->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control" id="status">
                                        <option {{ !empty($user->status) && $user->status == 1 ? 'selected' : '' }}
                                            value="1">Active
                                        </option>
                                        <option {{ !empty($user->status) && $user->status == 0 ? 'selected' : '' }}
                                            value="0">Inactive
                                        </option>
                                    </select>
                                </div>
